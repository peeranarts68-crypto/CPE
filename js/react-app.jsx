  ...
  useEffect(() => {
    ...
    try {
      ...
      unsub = fb.onSnapshot(settingsRef, (docSnap) => {
        if (docSnap.exists()) setSalesSettings(docSnap.data());
      }, (err) => console.log('Settings listener warning:', err));
    } catch (e) { console.log('Settings listener error:', e); }
    return () => unsub();
  }, []);
        ...
        <div className="modal-body" style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
          ...
          <div style={{ background: '#0a0b10', border: '1px solid var(--border-gold)', borderRadius: '12px', padding: '16px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px', alignItems: 'end' }}>
              <div>
                <label style={{ display: 'block', color: 'var(--text-sub)', fontSize: '0.82rem', marginBottom: '6px' }}>เลือกโหมดเปิด-ปิดการขาย:</label>
                <div style={{ display: 'flex', gap: '6px' }}>
                  {[['auto', '⏱️ ตามเวลา', '#0284c7', '#38bdf8'], ['open', '🟢 เปิดขาย', '#16a34a', '#22c55e'], ['closed', '🔴 ปิดขาย', '#dc2626', '#ef4444']].map(([mode, label, bg, border]) => (
                    <button
                      key={mode}
                      onClick={() => { setSalesMode(mode); handleSaveSalesSettings(mode, customDeadline); }}
                      style={{
                        flex: 1, padding: '8px', borderRadius: '6px', fontSize: '0.78rem', fontWeight: 'bold',
                        border: salesMode === mode ? `1px solid ${border}` : '1px solid #333',
                        background: salesMode === mode ? bg : '#18181b',
              ...
              <div>
                <label style={{ display: 'block', color: 'var(--text-sub)', fontSize: '0.82rem', marginBottom: '6px' }}>กำหนดเวลาปิดรับออเดอร์ (โหมดนับถอยหลัง):</label>
                <input type="datetime-local" value={customDeadline} onChange={(e) => setCustomDeadline(e.target.value)}
                  style={{ width: '100%', padding: '8px 12px', background: '#18181b', border: '1px solid var(--border-gold)', borderRadius: '6px', color: '#fff', fontSize: '0.85rem', boxSizing: 'border-box' }}
                />
              </div>
              <div>
                <button
                  ...
                  style={{ width: '100%', padding: '9px 16px', background: 'linear-gradient(135deg, #f5d061, #d4af37)', color: '#000', border: 'none', borderRadius: '6px', fontSize: '0.85rem', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 4px 12px rgba(245,208,97,0.3)' }}
                >
